/**
 * setup-db.js - Initialize the Kuzu database with schema and sample data
 */
...
// Schema creation helper function
const executeQuery = async (query, params = {}) => {
  try {
    console.log(`Executing: ${query}`);
    // Use execute method for non-blocking operations with proper callback
    const result = conn.execute(query, params);
...
async function setupDatabase() {
  try {
    ...
    // Insert sample data - always insert for now since we're setting up fresh
    const personCount = await executeQuery(`MATCH (p:Person) RETURN COUNT(*) as count`);
    if (!personCount || !personCount[0] || personCount[0].count === 0) {
